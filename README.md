...
cursos, home, dashboard e aula do estudante. As seis operações do cliente
tentam `/api/v1` primeiro e voltam ao respetivo action legado somente quando a
rota versionada não existe no ambiente. Falhas reais de autenticação,
autorização ou infraestrutura não acionam uma segunda chamada.
