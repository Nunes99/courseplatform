...
cliente utiliza essas rotas para curso, media, home, dashboard e aula, com
fallback controlado por operação para deploys anteriores.
...
Os métodos `publicCourseConfig`, `publicMediaConfig`, `studentHome`, `dashboard`
e `getLesson` em `public/api.js` consomem essas rotas. Cada um repete a leitura
pelo seu action apenas para `404/405` que identifique uma rota ainda não
instalada. Erros de domínio e infraestrutura são devolvidos sem fallback,
